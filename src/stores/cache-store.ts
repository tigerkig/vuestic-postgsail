...
export const useCacheStore = defineStore('cache', {
  ...
  actions: {
      ...
      if (navigator.onLine) {
        if (now - entry.ts > ttl) {
          ...
          if (assertion[0](entry.data)) {
              parent[index] = entry
            console.log(entry.data, "1111111111111")
            console.log(index, "222222222222")
            console.log(parent[index], "33333333333")
          } else {
    ...
    async logs() {
      await this.getData(['log_get', '62'], assertions.notPopulatedArray)
